...
developed, of which the Genetic Algorithm (\gls{GA}), Differential
Evolution (\gls{DE}), Particle Swarm Optimization (\gls{PSO}) and
Adaptive Simulated Annealing (ASA) received more attention.
\\ \\
...
Moreover, their elegance and simplicity appealed to many
people~--~ a thorough understanding of other optimization
algorithms that existed at the time (which were much harder to
implement, and did not always find the \emph{global} optimum), was
no longer required, and problems could be optimized much faster
(and usually much better) than was previously possible.
...
\section{\gls{GA} (based on natural evolution)}
...
\\ \\
The original \gls{GA} used a \emph{binary} representation of the
population, i.e., each individual is represented by \emph{bits}
instead of real numbers. Crossover and mutation are also carried
out bit wise, that is
...
\section{\gls{DE} (based on globalized pseudo-derivatives)}
...
the power of the \gls{DE} algorithm.
\\ \\
Numerous variations on this basic algorithm exist. However, they
normally improve \gls{DE}'s performance only marginally, and as
such, they will not be mentioned here.
...
it is implemented in \gls{GODLIKE}. Do note that the algorithm
described above is by no means \emph{adaptive}~--~I just gave it
that title as a placeholder for future work to be carried out.
Adaptive \gls{SA} means that the algorithm automatically adjusts
the cooling schedule and the Boltzmann constant to optimize the
quality of the converged solution. But as of yet, that is not yet
implemented.}.
...
\section{\gls{PSO} (based on swarm intelligence)}
...
\\ \\
In theory it is quite possible to go to Mars in only one
week~--~just bring an enormous rocket (large accelerations aside).
...
\emph{compromise} between the two objectives, instead of the
...
from a \gls{GA}, using members from the subset $P_{i+1}$ as
parents.
...
\\ \\
The \gls{GA}, \gls{DE}, \gls{PSO} and ASA algorithm have various
operators that try to prevent this problem. In \gls{GA}, it is the
mutation operator that sometimes generates a solution very far
from the other population members, increasing its robustness. In
\gls{PSO}, it is the fact that sometimes solutions get assigned
very large changes in their velocities, especially when being very
far removed from the attractors. However, as found in the
literature, premature convergence still frequently occurs and
necessitates several re-runs to make sure the global optimum is
found.
\\ \\
...
rather than go at it ``blindly'' and just use a \gls{GA} to solve
it.
\\ \\
...
optima that could almost never be found by either \gls{GA},
\gls{DE}, ASA or \gls{PSO} individually, \emph{could} be found by
but their \emph{combined} efforts in GODLIKE.
...
are to be used in the algorithms \gls{GA}, \gls{DE}, and
\gls{PSO}, the algorithms get assigned for instance [45, 13, 42]
or [6, 22, 72] individuals. Note that these numbers are chosen
differently at every iteration.
\\ \\
...
\begin{description}
  ...
  \item[algorithm] the optimization algorithm used (either '\gls{GA}',
  '\gls{PSO}', '\gls{DE}' or 'ASA').
...
\begin{description}
  ...
  bounds \texttt{lb} and \texttt{ub}. Also, for \gls{PSO} specifically,
  the bounds on the new velocities are checked.

  \item[initialize\_algorithms (Hidden)] \gls{PSO} and ASA need some
...
\texttt{non\_dominated\_sort} and \texttt{tournament\_selection}
instead of \texttt{replace\_parents}, and the \gls{PSO}-algorithm
must be initialized and used differently (see below).

\section{\gls{PSO} in Multi-objective Optimization}
Using the ASA and \gls{DE} optimizers (instead of the usual
\gls{GA}) to create offspring populations for the NSGA-II method
is quite straightforward. However, the \gls{PSO} algorithm is more
demanding; it needs to have some values for $lbest$, $gbest$ and
$nbest$, which are not defined in an obvious way for
multi-objective optimization. There are several papers in the
literature that deal with this very problem. Some find reasonable
results, but most perform quite poorly compared to similar
algorithms. The best one I could find relied on a completely
different algorithm (so \emph{not} NSGA-II), so for the time
being, I chose to use another, worse one, that \emph{did} fit in
GODLIKE's NSGA-II context.
\\ \\
But, it is particularly in this context that \gls{PSO} seems
rather unsuited for the task. To remedy the problem with $lbest$,
$gbest$ and $nbest$, the following criteria seemed to work best:
...
Despite these remedies, \gls{PSO} remains the least powerful
algorithm for multi-objective optimization; it doesn't really seem
to be able to achieve convergence.

\section{Known Problems and Issues} As mentioned above, using the \gls{PSO}
...
selection. It is very hard to get convergence with \gls{PSO}, but
in conjunction with \gls{DE} it does work reasonable.
\\ \\
The 'A' in (A)\gls{SA} is really not deserved. Basically I just
used the simulated annealing written by Joachim Vandekerckhove
(also on the FEX, file ID\#10548) and rewrote it to be suited for
populations. Adapting the control parameters at each iteration,
which is the true power of ASA, is not included now. As such it is
probably the weakest algorithm for single-objective optimization,
and the second-worst in multi-objective problems. However, I found
it to still be useful sometimes, as it generates many solutions in
\emph{all} regions of low function value.
\\ \\
For very large population sizes, the crossover operator in
\gls{GA} seems to take up very much computation time. I have no
idea why that is, so if you do know why, please let me know. Also,
the to-and-fro conversion between binary and real representations
is relatively costly. While this is probably not a problem (the
...
\\ \\
Naturally, I need to do some more research on using \gls{PSO} in
